...
// プッシュ通知送信
async function sendPushNotificationToUser(db: admin.firestore.Firestore, userId: string, notification: {
  title: string;
  body: string;
  data?: any;
}): Promise<void> {
  try {
    ...
    const tokens: string[] = devicesSnapshot.docs.map(doc => doc.id).filter(Boolean);
    ...
    response.responses.forEach((resp, idx) => {
      if (!resp.success) {
        const code = (resp.error && (resp.error as any).errorInfo && (resp.error as any).errorInfo.code) || '';
        if (code.includes('registration-token-not-registered') || code.includes('invalid-argument')) {
          const token = tokens[idx];
          deletions.push(db.collection('users').doc(userId).collection('devices').doc(token).delete());
        }
